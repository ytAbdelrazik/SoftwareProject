...
export class UserService {
  ...
  async addCoursesToStudent(userId: string, courseIds: string[]): Promise<any> {
    // Fetch the student with the userId
    const student = await this.studentModel.findOne({ userId }).populate('enrolledCourses').exec();

    if (!student) {
        throw new NotFoundException(`Student with ID ${userId} not found`);
    }

    // Extract existing course IDs from the `enrolledCourses` (assuming they are Mongoose ObjectIds or similar)
    const existingCourseIds = student.enrolledCourses.map((course: any) => course._id.toString());

    // Merge the existing course IDs with the new ones, avoiding duplicates
    const mergedCourseIds = Array.from(new Set([...existingCourseIds, ...courseIds]));

    // Update the student's `enrolledCourses` with the merged list of course IDs
    student.enrolledCourses = mergedCourseIds as any; // Cast as appropriate if required
